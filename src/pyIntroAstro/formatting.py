...
def Qprint(quantities_in, sigfig=3, style='s'):
    """ wrap up the syntax for printing astropy Quantity objects in a pretty 
    way. Other options for style inclue 'latex' """

    ''' Take in a scalar or iterable (list, array) of Quantity objects
    and return a formatted string, either scalar or list ''' 
    
    if not is_iterable(quantities_in):
        # Make it iterable
        quantities = [quantities_in.copy()]
    else:
        quantities = quantities_in.copy()
    ...
    for quantity in quantities:
        ...
        x.append(fmtstr.format(quantity))

    # If it's a scalar, re-extract it, otherwise leave as list
